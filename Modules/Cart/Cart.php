...
class Cart extends DarryldecodeCart implements JsonSerializable
{
    ...
    public function total()
    {
        if(Auth::user()){
            ...
            if(Auth::user()->hasMembership){
                 return $this->subTotal()
                        ...
                        ->subtract($this->membershipDiscount())
                        ->add($this->tax());
            }else{
                 return $this->subTotal()
                        ->add($this->shippingMethod()->cost())
      ...
      public function membershipDiscount(){
        ...
        $d =  $this->items()->sum(function ($cartItem) {
            if($cartItem->product->member_price->amount()>0){

                return round($cartItem->product->selling_price->subtract($cartItem->product->member_price)->multiply($cartItem->qty)->amount());
            }else{
                return 0;
            }
        });

        return Money::inDefaultCurrency($d);
    }
