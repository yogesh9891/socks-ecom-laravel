    ...
    >
        <section class="product-details-wrap">
            <div class="container">
                                ...
                                <div class="product-price" v-html="price">
                                    ...
                                    <div class="d-block mt-1">  
                                                @if($product->member_price->amount())
                                                    ...
                                                    {!! $product->formatted_member_price !!} <a href="{{ url('membership') }}"class="join_now_bt">Join Now</a>
                                                @endif
                                            </div>
                                <form
                                ...
                                >
                                    ...
                                    <div class="product-variants">
                                        @foreach ($product->options as $option)
                                             @if ($loop->odd)
                                                  @includeIf("public.products.show.custom_options.option_color")
                                             @else
                                                  @includeIf("public.products.show.custom_options.size")
                                             @endif
                                        @endforeach
                ...
                <div class="flex-column-reverse flex-lg-row">
...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        $( ".color_click" ).each(function( index ) {
            if($(this).hasClass('focus')){
                  let a = $(this).attr('color');
                 $('#product-color').text(a);
            }

    });
    $('.color_click').mouseover(function () {
        ...
        $('.color_click').removeClass('focus');
        $(this).addClass("focus");
        if(!$(this).hasClass('color-disabled')){
                let label = $(this).attr('color');
                  $.ajax({
                url: '{{ route('products.show.checkOptionAvailability',$product) }}',
                type: 'post',
                data: {type:'color',label:label},
                success: function (data) {
                    ...
                    if(data.success){
                        ...
                        $('.notices.is-bottom').html('<div role="alert" class="toast toast-default is-bottom-right" style=""><p class="toast-text">Product Not Found in Stock !!!</p></div>');
        ...
        $('.size-containt').click(function () {
        $('.size-containt').removeClass('focus');
        $(this).addClass("focus");

            if(!$(this).hasClass('size-disabled')){
                let label = $(this).attr('size');
                let color = $('.color-disabled').attr('color');
                  $.ajax({
                url: '{{ route('products.show.checkOptionAvailability',$product) }}',
                type: 'post',
                data: {type:'size',label:label,color:color},
                success: function (data) {
