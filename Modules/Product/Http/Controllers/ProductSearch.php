...
trait ProductSearch
{
    ...
    public function searchProducts(Product $model, ProductFilter $productFilter)
    {
        ...
        $query = $model->filter($productFilter)->where('parent_id',null);
        ...
        foreach ($products as $key => $value) {
            if($value->is_product_configurable != 0){
                  $p = Product::where('parent_id',$value->id)->first();
                  $products[$key]->slug = $p->slug;
